        ...
        for change in mr.changes()["changes"]:
            if "diff" in change:
                try:
                    ...
                    print(
                        f"Error getting file content for {change['new_path']}: {str(e)}"
                    )
                    all_content.append(
                        f"File: {change['new_path']}\n\nDiff:\n{change['diff']}\n{'-' * 16}"
                    )
...
if args.mode == "general":
    ...
    if mr.state == "opened":
        mr.notes.create({"body": f"{review_text}"})
        print("\n===")
        print("General MR Review posted as a comment in the MR.")
    else:
        print("MR is closed, review not posted.")
                ...
                try:
                    ...
                    if file_review and "no feedback" not in file_review.lower():
                        ...
                        try:
                            ...
                            print(
                                f"Posted comment on {change['new_path']} at line {line_num}: {comment}"
                            )
                        except Exception as e:
                            print(
                                f"Error posting comment on {change['new_path']}: {str(e)}"
                            )
                except Exception as e:
